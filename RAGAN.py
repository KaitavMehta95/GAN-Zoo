...
if __name__ == '__main__':
    ...
    # Initialize RaGAN loss function
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            y_pred_fake = netD(fake.detach()).view(-1)

            D_G_z1 = y_pred_fake.mean().item()
            # Add the gradients from the all-real and all-fake batches
            y = torch.full((b_size,), real_label, device=device)
            y2 = torch.full((b_size,), fake_label, device=device)
            errD = (BCE_stable(y_pred - torch.mean(y_pred_fake), y) + BCE_stable(y_pred_fake - torch.mean(y_pred),
                                                                                 y2)) / 2
            errD.backward()
            # Update D
            optimizerD.step()
            ...
            if (iters % 500 == 0) or ((epoch == num_epochs - 1) and (i == len(dataloader) - 1)):
                ...
                path = 'C:/Users/RACHIT/Desktop/KaitavProject/LSGAN/mnist_dcgan/ragan/'
                size_figure_grid = 5
